...
class SentimentStrategy(BaseStrategy):
    ...
    async def generate_signal(
        self, price_history, portfolio_value, token_balances,
    ):
        ...
        btc_mom = (
            (btc_prices[-1] - btc_prices[-3]) / btc_prices[-3]
            if btc_prices[-3] > 0
            else 0
        )
        eth_mom = (
            (eth_prices[-1] - eth_prices[-3]) / eth_prices[-3]
            if eth_prices[-3] > 0
            else 0
        )
        ...
        if btc_mom > 0.01 and eth_mom > 0.005:
            ...
            if usdc_bal > settings.min_trade_usd:
                return TradeSignal(
                    ...
                    from_symbol="USDC",
                    to_symbol="WETH",
                    strength=min(btc_mom * 20, 0.8),
                    reason=(
                        f"Risk-on: BTC+{btc_mom:.2%} ETH+{eth_mom:.2%}"
                    ),
        ...
        if btc_mom < -0.01 and eth_mom < -0.005:
            ...
            if eth_bal > settings.min_trade_usd:
                return TradeSignal(
                    ...
                    from_symbol="WETH",
                    to_symbol="USDC",
                    strength=min(abs(btc_mom) * 20, 0.8),
                    reason=(
                        f"Risk-off: BTC{btc_mom:.2%} ETH{eth_mom:.2%}"
                    ),
                    strategy_name=self.name,
                )
